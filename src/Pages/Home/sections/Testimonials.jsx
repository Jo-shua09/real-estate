        ...
        <div className="border-2 py-10 px-8 border-white/5 rounded-xl w-full p-6 h-[25rem] flex flex-col gap-y-7">
          <div className="flex items-center gap-x-1">
            {[...Array(5)].map((_, index) => (
              <Star
                key={index}
                className={`bg-white/5 border border-white/5 p-2 rounded-full !text-5xl ${index < testimonial.rating ? "text-yellow-500" : "text-white/20"}`}
              />
            ))}
          </div>
          <p className="my-3 normal-case text-2xl font-medium">{testimonial.comment}</p>
          <div className="flex justify-between items-end w-full">
            <div className="flex items-center gap-x-4">
              <img src={testimonial.avatar} alt={testimonial.author} className="w-24 h-24 rounded-full object-cover" />
              <div>
