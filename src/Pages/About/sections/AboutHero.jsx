  ...
  return (
    <div className="w-full h-full flex items-center flex-wrap-reverse md:flex-nowrap gap-x-20 justify-between gap-y-10 section-page">
      <div className="md:flex-1 w-full">
        <img src={Abstract} alt="abstract image" className="w-fit h-fit object-cover" />
        <h1 className="text-5xl my-5 font-semibold">our journey</h1>
        <p className="text-white/70 text-2xl normal-case font-medium mb-3">
          Our story is one of continuous growth and evolution. We started as a small team with big dreams, determined to create a real estate platform that
          transcended the ordinary. Over the years, we've expanded our reach, forged valuable partnerships, and gained the trust of countless clients.
        </p>
      ...
      <div className="md:flex-1 w-full mt-5 md:m-0 bg-hero bg-no-repeat md:rounded-none !rounded-xl border-2 border-white/5">
        <div className="w-full h-[40rem] md:rounded-none rounded-xl border-2 border-white/5">
          <img src={AboutImage} alt="hero image" className="w-full h-full object-cover" />
        </div>
      </div>
    </div>
  );
